...
int main(int argc, char **argv) {
    ...
    cin >> k;
    //-Experiment config
    //matrix_t mat_A = {.height = m, .width = k, .dataLength = m * k};
    //matrix_t mat_B = {.height = k, .width = n, .dataLength = n * k};
    //matrix_t mat_C = {.height = mat_A.height, .width = mat_B.width, .dataLength = m * n};
    matrix_t mat_A{};
    matrix_t mat_B{};
    matrix_t mat_C{};
    mat_A.height = m;
    ...
    ZeroCopyMem<float> a = init_zero_copy_region<float>(cl, mat_A.dataLength);
    ZeroCopyMem<float> b = init_zero_copy_region<float>(cl, mat_B.dataLength);
    ZeroCopyMem<float> c = init_zero_copy_region<float>(cl, mat_C.dataLength);
    mat_A.data = a.hostPtr;
    mat_B.data = b.hostPtr;
    mat_C.data = c.hostPtr;
    randArrayGenerator<float>(0.0f, 1.0, mat_A.data, mat_A.dataLength);
    randArrayGenerator<float>(0.0f, 1.0, mat_B.data, mat_B.dataLength);
    ...
    printf("A(%ld, %ld) x B(%ld, %ld) = C(%ld, %ld)\n",
           mat_A.height, mat_A.width, mat_B.height, mat_B.width, mat_C.height, mat_C.width);
    std::cout << "A: " << std::endl;
    printMatrix(mat_A.data, mat_B.height, mat_A.width);
    std::cout << "B: " << std::endl;
    printMatrix(mat_B.data, mat_B.height, mat_B.width);

    double runtimeGPU = getCurrentTime();
    cl_wrapper::checkError(clSetKernelArg(kernel_8x4, 0, sizeof(cl_mem), &a.deviceBuffer));
    cl_wrapper::checkError(clSetKernelArg(kernel_8x4, 1, sizeof(cl_mem), &b.deviceBuffer));
    cl_wrapper::checkError(clSetKernelArg(kernel_8x4, 2, sizeof(cl_mem), &c.deviceBuffer));
    ...
    //Execute 2nd kernel
    cl_wrapper::checkError(clSetKernelArg(kernel_rem, 0, sizeof(cl_mem), &a.deviceBuffer));
    cl_wrapper::checkError(clSetKernelArg(kernel_rem, 1, sizeof(cl_mem), &b.deviceBuffer));
    cl_wrapper::checkError(clSetKernelArg(kernel_rem, 2, sizeof(cl_mem), &c.deviceBuffer));
    ...
    cout << "GPU Run time: " << runtimeGPU / LOOP_TIME << "s" << endl;
    ...
    for (int i = 0; i < LOOP_TIME; ++i) {
        ...
        runtimeCPU += gemm_cpu(
                0, 0, m, n, k, 1,
                mat_A.data, static_cast<int>(mat_A.width),
                mat_B.data, static_cast<int>(mat_B.width),
                1,
                cpu_c, static_cast<int>(mat_C.width));
    }
    std::cout << "CPU Run time: " << runtimeCPU / LOOP_TIME << "s" << std::endl;

    std::cout << "CPU/GPU: " << runtimeCPU / runtimeGPU << std::endl;

    std::cout << "Diff: " << calVecDiff(cpu_c, mat_C.data, m * n) << "\n";
    std::cout << "CPU:" << std::endl;
    printMatrix(cpu_c, static_cast<int>(mat_C.height), static_cast<int>(mat_C.width));
    std::cout << "GPU:" << std::endl;
    printMatrix(mat_C.data, static_cast<int>(mat_C.height), static_cast<int>(mat_C.width));
