...

{
    ...
    if (fork()) {
       ...
       // };
        ...
        ptr = static_cast<char *>(mmap(nullptr, len, prot, map_flags, share_fd, 0));
        if (ptr == MAP_FAILED) {
            return;
        }
        strcpy(ptr, "master");
        ...
        *(int *)CMSG_DATA(cmsg) = share_fd;
        ...
        printf("child %d\n", recv_fd);
        fd = ion_open();
        ptr = static_cast<char *>(mmap(nullptr, len, prot, map_flags, recv_fd, 0));
        if (ptr == MAP_FAILED) {
            return;
        }
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        switch (c) {
        case 'l':
            len = static_cast<size_t>(atol(optarg));
            break;
        case 'g':
            align = static_cast<size_t>(atol(optarg));
            break;
        case 'z':
            map_flags = 0;
        ...
        case 'p':
            ...
            break;
        case 'f':
            alloc_flags = static_cast<unsigned int>(atol(optarg));
            break;
        case 'h':
            heap_mask = static_cast<unsigned int>(atol(optarg));
            break;
        case 'a':
            test = ALLOC_TEST;
            break;
    ...
    //}
    printf("ion_alloc_test...\n");
    ion_alloc_test();
    printf("ion_map_test... \n");
    ion_map_test();
    printf("ion_share_test...\n");
    ion_share_test();
    return 0;
}
